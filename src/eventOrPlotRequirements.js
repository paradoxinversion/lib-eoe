...
const requirementEoECitizenAvailable = (gameData) => {
  return getCitizens(gameData.people, gameData.player.empireId, true).length > 0 ? true : false;
}

const requirementEoECitizenInZoneAvailable = (gameData, requirementParams) => {
  const zoneId = requirementParams.zoneId;
  return getZoneCitizens(gameData.people, zoneId, true).length > 0 ? true : false;
}
